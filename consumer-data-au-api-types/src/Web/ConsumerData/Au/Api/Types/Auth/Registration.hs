...
import Crypto.JOSE.JWS (HeaderParam(..), kid, newJWSHeader, x5t, x5tS256)
import Crypto.JOSE.Types (Base64SHA1(..), Base64SHA256(..))
import Crypto.JWT
  ( AsJWTError
  , Audience
  , ClaimsSet
...
import Data.Text.Encoding as TE (decodeUtf8, encodeUtf8)
...
newtype FapiGrantTypes =
  FapiGrantTypes GrantTypes
  deriving (Generic, Show, Eq)
...
_FapiGrantTypes =
  prism'
    ...
    (\(GrantTypes grantTypes) ->
       if grantTypes `isSubsetOf` permittedGrantTypes && requiredGrantTypes `isSubsetOf`
          grantTypes
...
newtype FapiApplicationType =
  FapiApplicationType ApplicationType
...
newtype RegistrationContacts =
  RegistrationContacts (Set EmailAddress)
  deriving (Generic, Show, Eq)
...
instance FromJSON RegistrationContacts where
  parseJSON =
    fmap RegistrationContacts . parseSpaceSeperatedSet _Unwrapped "EmailAddress"
...
data SubjectType =
  Pairwise -- `Public` type not supported
  deriving (Generic, Show, Eq)
...
data JwkSet
  = JwksRef JwksUri --  ^ @jwks_uri@ parameter that specifies a URL for the client's JSON Web Key Set for pass-by-reference
...
tokEndPtMethMap =
  ...
  where
    ...
    setApm' a =
      M.empty & at "token_endpoint_auth_method" ?~ toJSON (a :: T.Text)
...
instance FromJSON TokenEndpointAuthMethod where
  parseJSON =
    parseJSONWithPrism _TokenEndpointAuthMethod "TokenEndpointAuthMethod"
...
newtype TlsClientAuthSubjectDn =
  TlsClientAuthSubjectDn T.Text
  deriving (Generic, ToJSON, FromJSON, Show, Eq)
...
newtype FapiTokenEndpointAuthMethod =
  FapiTokenEndpointAuthMethod TokenEndpointAuthMethod
  deriving (Generic, Show, Eq)
...
_FapiTokenEndpointAuthMethod =
  prism
    ...
    (\case
       PrivateKeyJwt f ->
         Right . FapiTokenEndpointAuthMethod . PrivateKeyJwt $ f
       TlsClientAuth f ->
         Right . FapiTokenEndpointAuthMethod . TlsClientAuth $ f
       e -> Left e)
...
instance FromJSON RedirectUrls where
  parseJSON =
    fmap RedirectUrls .
    parseSpaceSeperatedSet (_URI . _Unwrapped) "RedirectUrls"
...
_RedirectUrls =
  prism'
    (\(RedirectUrls r) -> r)
    (\uris ->
       if not (null uris) && allValid uris
...
newtype FapiResponseTypes =
  FapiResponseTypes (Set ResponseType)
  deriving (Generic, Show, Eq)
...
instance FromJSON FapiResponseTypes where
  parseJSON =
    fmap FapiResponseTypes .
    parseSpaceSeperatedSet responseTypeText "FapiResponseTypes"
...
newtype FapiAcrValues =
  FapiAcrValues T.Text
  deriving (Generic, ToJSON, FromJSON, Show, Eq)

newtype MutualTlsSCAT =
  MutualTlsSCAT Bool
  deriving (Generic, ToJSON, FromJSON, Show, Eq)
...
newtype NotificationEndpoint =
  NotificationEndpoint HttpsUrl
  deriving (Generic, ToJSON, FromJSON, Show, Eq)
...
data JwsRegisteredClaims = JwsRegisteredClaims
  { _iss :: ClientIss -- ^ The name of the RP. RFC7591 2.3 mandates issuer claim is present if supplied in a software statement.
  , _aud :: Audience -- ^ The OP.
  , _iat :: NumericDate -- ^ Issued at.
  , _exp :: NumericDate -- ^ Expiration time.
  , _jti :: JTI -- ^ JWT ID.
  } deriving (Generic, Show, Eq)
...
data ClientMetaData = ClientMetaData
  ...
  , _applicationType :: Maybe FapiApplicationType -- ^ Kind of the application. CDR mandates this to be just @web@ (i.e no @native@ apps are allowed).
  ...
  , _clientName :: Maybe Script -- ^ Human-readable name of the client to be presented to the end user. Mandatory field according to CDR.
  ...
  , _subjectType :: Maybe SubjectType -- ^ Requested for responses to this client. Mandatory field according to CDR.
  ...
  , _mutualTlsSenderConstrainedAccessTokens :: Maybe MutualTlsSCAT -- ^ @mutual_tls_sender_constrained_access_tokens@ indicating the client's intention to use mutual TLS sender constrained access tokens; CDR required field that must always be `true`.
  , _clientNotificationEndpoint :: Maybe NotificationEndpoint -- ^ @client_notification_endpoint@ - CDR required URI for CIBA callback.
  } deriving (Generic, Show, Eq)

-- TODO: Add smart constructor to only allow supply of encoded SS, depending on the CDR spec
data RegoReqSoftwareStatement
  = EncodedSs T.Text
  | DecodedSs SoftwareStatement
  deriving (Show, Eq)
...
data RegReqAccessToken =
  RegReqAccessToken RegistrationAccessToken
                    RegistrationClientUri

newtype RegistrationAccessToken =
  RegistrationAccessToken T.Text

newtype RegistrationClientUri =
...
newtype ClientSecretExpiresAt =
  ClientSecretExpiresAt NumericDate

-- | Time at which the Client Identifier was issued.
newtype ClientIdIssuedAt =
  ClientIdIssuedAt NumericDate
...
newtype RegistrationErrorDescription =
  RegistrationErrorDescription T.Text
...
metaDataToAesonClaims ClientMetaData {..} =
  M.empty & at "client_name" ?~ toJSON _clientName & at "client_uri" .~
  (toJSON <$> _clientUri) &
  at "contacts" .~
  (toJSON <$> _contacts) &
  at "logo_uri" .~
  ...
  (<> (_TokenEndpointAuthMethod .
       _FapiTokenEndpointAuthMethod # _tokenEndpointAuthMethod)) &
  at "scope" .~
  (toJSON <$> _scope) &
  at "software_id" .~
  (toJSON <$> _softwareId) &
...
regoReqToJwt jwk rr =
  let mkCs h m =
      ...
      jwsHead =
        newJWSHeader ((), _FapiPermittedAlg # _alg rrh) & kid ?~
   ...
   in do ssb64 <-
           case _regReqsoftwareStatement rr of
...
setRegisteredClaims h claims =
  claims & claimIss .~ (getClientIss <$> _iss h) & claimAud .~ _aud h & claimIat .~
  _iat h &
  claimJti .~
  (getJTI <$> _jti h) &
  claimExp .~
...
jwtToRegoReq audPred issPred jwk jwt
 ...
 = do
  ...
  ssjwt <-
    decodeCompact =<< BSL.fromStrict . TE.encodeUtf8 <$> claims ^.
    unregisteredClaims .
  ...
  RegistrationRequest <$> jwsHead <*> getRegisteredClaims claims <*> c2m claims <*>
    pure (DecodedSs ss)
...
getClaim ::
     forall e m a. (AsError e, MonadError e m, FromJSON a)
  => AesonClaims
  -> T.Text
  -> m a
...
aesonClaimsToMetaData ::
     forall e m. (AsError e, MonadError e m)
  => AesonClaims
  -> m ClientMetaData
