...
, hedgehog, http-client, jose, lens, memory, mmorph, modern-uri
...
mkDerivation {
  ...
  libraryHaskellDepends = [
    ...
    contravariant country cryptonite dependent-map dependent-sum
    dependent-sum-template digit errors exceptions ghc-prim jose lens
    memory mmorph modern-uri monad-time mtl nat network-uri
    semigroupoids servant servant-client servant-client-core
    servant-waargonaut tagged template-haskell text time transformers
    unordered-containers waargonaut
  ];
  testHaskellDepends = [
    aeson aeson-diff aeson-pretty attoparsec base bytestring containers
    country dependent-map dependent-sum digit exceptions hedgehog
    http-client jose lens modern-uri mtl network-uri profunctors
    servant servant-client servant-server servant-waargonaut tagged
    tasty tasty-discover tasty-golden tasty-hedgehog tasty-hunit text
    time transformers waargonaut wai warp
  ];
